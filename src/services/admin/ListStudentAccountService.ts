...
export class ListStudentAccountService {
    async GetAllProfileStudent(search: string, isApproved?: number, pageIndex?: number, pageSize?: number): Promise<PageResult<StudentProfile> | null> {
        try {
            ...
            const response = await axiosPrivateProfileServcie.get<ApiResponse<PageResult<StudentProfile>>>('/v1/admin/student/list', {
                params: {
    ...
    async GetProfileStudentById(id: string): Promise<StudentProfile | null> {
        try {
            // Gọi API để lấy danh sách sinh viên
            const response = await axiosPrivateProfileServcie.get<ApiResponse<StudentProfile>>('/v1/admin/student/detail?id=' + id, {
            });
    ...
    async BanStudent(profileId: string, reason?: string): Promise<boolean> {
        try {
            ...
            const response = await axiosPrivateProfileServcie.put<ApiResponse<any>>('/v1/admin/student/ban', requestBody);
