...
export class BusinessDetailService {
    async GetBusinessDetail(id?: string): Promise<BusinessProfilesDTO | null> {
        ...
        try {
            ...
            const response = await axiosPrivateProfileServcie.get<ApiResponse<any>>('/v1/business/business-profile2/' + id);
    ...
    async GetListJob(id?: string, pageIndex?: number, pageSize?: number): Promise<PaginatedResponse<JobPostingsResponseDTO> | null> {
        ...
        try {
            ...
            // Gửi PUT request tới API
            const response = await axiosRecruitment.get<ApiResponse<PaginatedResponse<JobPostingsResponseDTO>>>('job-postings/getalljobbybusinessid/' + id+'?pageIndex='+pageIndex+'&pageSize='+pageSize);
